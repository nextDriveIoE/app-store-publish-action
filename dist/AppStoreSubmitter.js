...
exports.AppStoreSubmitter = void 0;
const appstore_connect_sdk_1 = require("appstore-connect-sdk");
const openapi_1 = require("appstore-connect-sdk/openapi");
const fs_1 = __importDefault(require("fs"));
class AppStoreSubmitter {
    constructor(config) {
        const privateKey = fs_1.default.readFileSync(config.privateKeyPath, 'utf8');
        this.client = new appstore_connect_sdk_1.AppStoreConnectAPI({
     ...
     */
    async getTestFlightVersion(appId, versionString) {
        try {
            console.log(`搜尋 TestFlight 版本 ${versionString}...`);
            const buildsApi = await this.client.create(openapi_1.BuildsApi);
            ...
            const targetBuild = buildsResponse.data.find(build => {
                if (buildsResponse.included) {
                    ...
                    if (preReleaseVersionId) {
                        const preReleaseVersion = buildsResponse.included.find(item => item.type === 'preReleaseVersions' && item.id === preReleaseVersionId);
                        if (preReleaseVersion?.attributes?.version === versionString) {
                            return true;
                        }
                    }
                }
     ...
     */
    async submitTestFlightBuildForAppStore(buildId, versionString) {
        try {
            ...
            const appsApi = await this.client.create(openapi_1.AppsApi);
            let appStoreVersion;
            try {
                // 先嘗試取得現有的版本
                ...
                if (versionsResponse.data && versionsResponse.data.length > 0) {
                    ...
                    const versionsApi = await this.client.create(openapi_1.AppStoreVersionsApi);
     ...
     * 驗證版本狀態是否可以送審
     */
    validateVersionState(currentState, versionString) {
     ...
     * 建立 App Store 版本送審請求
     */
    async createSubmissionRequest(appId, versionString, versionId) {
        console.log(`建立送審請求, appId = ${appId}, versionString = ${versionString}`);
        try {
            // 切換到送審狀態
            const reviewSubmissionsApi = await this.client.create(openapi_1.ReviewSubmissionsApi);
            try {
            ...
            // // 建立送審請求
            const reviewSubmissionItemsApi = await this.client.create(openapi_1.ReviewSubmissionItemsApi);
    ...
    async updateWhatNews(versionId, whatNews) {
        const versionsApi = await this.client.create(openapi_1.AppStoreVersionsApi);
        // 取得指定版本的所有本地化資訊
        ...
        const localizationsApi = await this.client.create(openapi_1.AppStoreVersionLocalizationsApi);
        for (const localization of localizations) {
            console.log(`更新版本 ${versionId} 的本地化資訊: ${localization.id}, locate = ${localization.attributes?.locale}`);
            const whatsNew = whatNews[this.mapToLocale(localization.attributes?.locale)];
     ...
     * 檢查是否為已送審的錯誤
     */
    async isAlreadySubmittedError(error) {
     ...
     * 更新版本狀態為準備送審
     */
    async updateVersionForSubmission(versionId, versionString) {
     ...
     * @param versionString - 版本號 (例如: "2.0.1303")
     */
    async submitForReview(appId, versionString, whatNews) {
        try {
            ...
            const currentState = appStoreVersion.attributes?.appStoreState;
            // 2. 檢查版本狀態是否可以送審
...
exports.AppStoreSubmitter = AppStoreSubmitter;
